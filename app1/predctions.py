...
class DiseasePrediction:
    def __init__(self):
        ...
        with open(r"C:\Program Files\JetBrains\projects\MediPlus\disease_model.pkl", "rb") as file:
            data = pickle.load(file)

        self.model = data.get("model")
        self.onehot_encoders = data.get("onehot_encoders")
        self.le_disease = data.get("le_disease")
        self.feature_names = data.get("feature_names")

    def predict_disease(self, symptoms):
        try:
            # Convert symptoms to DataFrame
            input_df = pd.DataFrame({col: [symptom] for col, symptom in zip(self.onehot_encoders.keys(), symptoms)})

            # Apply One-Hot Encoding
            encoded_dfs = []
            for col in self.onehot_encoders:
                ohe = self.onehot_encoders[col]
                encoded_values = ohe.transform(input_df[[col]])
                encoded_df = pd.DataFrame(encoded_values, columns=[f"{col}_{v}" for v in ohe.categories_[0]])
                encoded_dfs.append(encoded_df)

            # Combine encoded features
            input_encoded = pd.concat(encoded_dfs, axis=1)
